...
const cryptoMocks = vi.hoisted(() => ({ decryptWebhookValue: vi.fn() }));
...
describe("notificações Chanify", () => {
  beforeEach(() => {
    ...
    cryptoMocks.decryptWebhookValue.mockReturnValue(JSON.stringify({ token: "chanify-token-123" }));
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: true, status: 200 }));
  });
  afterEach(() => vi.unstubAllGlobals());

  it("valida token e não aceita valores vazios", () => {
    expect(validateChanifyConfig({ token: " chanify-token-123 " })).toEqual({ token: "chanify-token-123" });
