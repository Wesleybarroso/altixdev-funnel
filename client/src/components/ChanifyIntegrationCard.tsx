...
export default function ChanifyIntegrationCard() {
  ...
  return <article className="rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_10px_30px_rgba(15,23,42,.04)]">
    ...
    <form onSubmit={event => { event.preventDefault(); save.mutate({ token: token || undefined, removeToken: false, enabled }); }} className="mt-6 space-y-4">
    ...
    <p className="mt-3 text-xs leading-5 text-slate-400">No Chanify: crie ou selecione um canal, abra os detalhes e copie o token. Cole-o somente neste campo.</p>
  </article>;
}
